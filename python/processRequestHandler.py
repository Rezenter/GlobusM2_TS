...
class Handler:
    ...
    def refresh_shots(self, req):
        ...
        resp['config'] = []
        tmp = sorted(os.listdir(DB_PATH + '/config_cpp/'), reverse=True)
        tmp.extend(sorted(os.listdir(self.config_path), reverse=True))
        for entry in tmp:
    ...
    def get_integrals_shot(self, req):
        ...
        if self.raw_processor is None or self.raw_processor.is_plasma != req['is_plasma'] or \
            ...
            self.raw_processor = raw_proc.Integrator(DB_PATH, int(req['shotn']), req['is_plasma'], req['config'])
        #print(self.raw_processor is None, self.raw_processor.is_plasma != req['is_plasma'], self.raw_processor.shotn != int(req['shotn']))
        resp = {
            'timestamps': [],
            'energies': []
        }
        if len(self.raw_processor.error) != 0:
            print(self.raw_processor.error)
            fuck
    ...
    def get_event_raw(self, req):
        ...
        for ch in self.raw_processor.config['poly'][poly_ind]['channels']:
            ...
            elif self.raw_processor.version >= 6:
                event.append(list([self.raw_processor.header['boards'][ch['adc']]['offset'] - 1250 + v * 2500 / 4096] for v in self.raw_processor.data[ch['adc']][int(req['event'])]['ch'][ch['ch']]))
        ...
        if self.raw_processor.version <= 1:
            ...
            las = (list([self.raw_processor.header['boards'][board_ind]['offset'] - 1250 + v * 2500 / 4096] for v in self.raw_processor.data[board_ind][int(req['event'])]['ch'][0]))
        elif self.raw_processor.version >= 3:
