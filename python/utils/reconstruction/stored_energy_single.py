...
while inner_data['ra'] + ra_step < 1:
    ...
    surfaces.append(inner_data)
    if outer_ra >= profile[len(ring_areas) + 1]['r/a']:
        ring_surf.append(inner_data)
        if len(ring_areas) == 0:
            ring_areas.append(area_int)
        else:
            ring_areas.append(area_int - sum(ring_areas))
#integrate outer

for i in range(len(ring_areas)):
    if i == 0:
        print(ring_areas[i]/origin_area[i], ring_areas[i], origin_area[i])
    else:
        print(ring_areas[i] / (origin_area[i] - origin_area[i - 1]), ring_areas[i], origin_area[i] - origin_area[i - 1])


with open('volume_check.csv', 'w') as out_fp:
    line = 'theta, '
    for surf in ring_surf:
